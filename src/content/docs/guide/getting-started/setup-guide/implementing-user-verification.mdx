...
<LinkButton href="https://github.com/released-software/released-embed-demo" variant="secondary" icon="external">NextJS demo</LinkButton>
<LinkButton href="/guide/resources/how-tos/how-to-implement-user-verification/" variant="secondary" icon="external">Express demo</LinkButton>
...
sequenceDiagram
    ...
    participant HubPortal as Hub Portal
    User ->> AppServer: Accesses the Hub portal
    AppServer ->> HubAPI: Requests auth token (ACCOUNT_ID, USER_EMAIL)
    HubAPI -->> AppServer: Returns AUTH_TOKEN
    AppServer -->> User: Embeds portal with AUTH_TOKEN
    User ->> HubPortal: Accesses portal with AUTH_TOKEN
    HubPortal ->> HubAPI: Verifies AUTH_TOKEN
    HubAPI -->> HubPortal: Authentication success/failure
    HubPortal -->> User: Grants or denies access
```

...
For Jira Assets fields, use `is any of`, `is none of`, `contains all`, `is
empty`, or `is not empty` to match linked objects. Filters compare object
identity, not object attributes. See [Jira Assets](/betterboard/shape-the-board/jira-assets/)
